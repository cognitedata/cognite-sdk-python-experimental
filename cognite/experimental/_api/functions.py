...
class FunctionsAPI(APIClient):
    ...
    def create(
        ...
        utils._auxiliary.assert_type(cpu, "cpu", [float], allow_none=False)
        utils._auxiliary.assert_type(memory, "memory", [float], allow_none=False)
        ...
        function = {
            ...
            "resources": {"cpu": str(cpu), "memory": str(memory)},
        }
        if external_id:
