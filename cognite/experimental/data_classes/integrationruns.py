        ...
        if len(self.statuses) > 0:
            for status in self.statuses:
                if not (status is None):
                    result.append(IntegrationRun(self.external_id, status["status"], status["createdTime"]))
        return result
...
class IntegrationRun(CogniteResource):
    ...
    Args:
        external_id (str): The external ID of relted integration provided by the client. Must be unique for the resource type.
