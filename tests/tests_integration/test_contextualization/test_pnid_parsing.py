...
class TestPNIDParsingIntegration:
    def test_run_detect_str(self):
        ...
        assert {"items", "requestTimestamp", "startTimestamp", "statusTimestamp", "fileId", "fileExternalId"} == set(job.result.keys())
    ...
    def test_run_convert(self):
        ...
        assert isinstance(job, ContextualizationJob)
        assert {"pngUrl", "requestTimestamp", "startTimestamp", "statusTimestamp", "svgUrl", "fileId", "fileExternalId"} == set(job.result.keys())
